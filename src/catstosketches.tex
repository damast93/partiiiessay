...
Note that by the required adjunction, we get
\[ \hom(F_!(yA),B) \cong \hom(yA,F^*(B)) \cong F^*(B)(A), \]
so $F^*(B)$ has to be defined as $\hom(F(-),B)$. \\

The way every presheaf is written as a colimit of representables has a canonical description. For $P : \cat A^\op \to \catname{Set}$, we let $\el(P)$ be the category $(y \downarrow P)$. By the Yoneda lemma, we have a forgetful functor $\pi : \el(P) \to \cat A$ and $P \cong \colim y\circ \pi$. \\

Let $y' : \cat A^\op \to \fun{\cat A}{\catname{Set}}$ be the contravariant Yoneda embedding.
...
\end{Proposition}
\begin{Proof}
\ref{item:elfiltered} $\Rightarrow$ \ref{item:fflat} is immediate. For \ref{item:fflat} $\Rightarrow$ \ref{item:kancont} let $F = \colim_i y'A_i$ be $\lambda$-filtered colimit of representables.
\end{Proof}

\begin{Corollary}
$\flat_\lambda(\cat A)$ is closed unter $\lambda$-filtered limits in $\fun{\cat A}{\catname{Set}}$.
\end{Corollary}
\begin{Proof}
Let $F = \colim_i F_i$ be $\lambda$-filtered colimit of $\lambda$-flat functors; we show that $F_!$ preserves $\lambda$-small limits. Let $(P_j)$ be a $\lambda$-small diagram of presheaves, then
\begin{align*}
F_!(\lim_j P_j) &\cong \colim_i (F_i)_!(\lim_j P_j) 
\cong \colim_i \lim_j (F_i)_!(P_j) 
\cong \lim_j \colim_i (F_i)_!(P_j) 
\cong \lim_j F_!(P_j).
\end{align*}
because each $(F_i)_!$ preserves $\lambda$-small limits and \ref{prop:smallvsfiltered}.
\end{Proof}

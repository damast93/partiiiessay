...
\item To give an example that is not equivalent to a variety -- the category $\catname{Pos}$ of posets is locally finitely presentable, as $\{\mathbf 2\}$ is a strong generator. % Why isn't that a variety?
...
Therefore, representable functors are finitely presentable objects, and by the Yoneda lemma form generator of the functor category. To see that this generator is strong, note that the monomorphisms $F \xrightarrow{\alpha} G$ are pointwise-injective transformations because $\catname{Set}$ has pullbacks. Thus a proper subobject $\alpha$ leads to some element $x \in GA \setminus \alpha_A(FA)$ and the transformation $y'A \xrightarrow{x} G$ doesn't factor through $F$. % This is a variety
\end{enumerate}
\end{Example}

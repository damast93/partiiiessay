...
The theory of varieties is easily extended to include \emph{many-sorted sets}, e.g. a graph is a two-sorted algebra on sorts $e,v$ with operations $s, t : e \to v$. Note than an $S$-sorted set $A$ is just a functor $A : S \to \catname{Set}$ with $S$ viewed a discrete category. Prescribing product cones for $A$ just allows us to declare certain sorts as products of other sorts, allowing us to reduce all operations to unary ones at the cost of introducing more sorts. 

...
This procedure gives an idea on how to turn any set of identities into a finite product sketch. We want see a converse with the following idea: An $S$-sorted set $A$ is just a functor $A : S \to \catname{Set}$ with $S$ viewed as a discrete category. When $S$ is a non-discrete category, the morphisms introduce operations between the sorts. \\

Prescribing product cones for $A$ additionally declares certain sorts as products of other sorts, thereby allowing us to reduce all operations to unary ones at the cost of introducing more sorts.
...
and
\[ f(g(x)) = (f \circ g)(x)  \]
for all morphisms $g : r \to s, f : s \to t$ in $\cat A$ where $x$ is a variable of sort $r$. We can now think of algebras as functors $F : \cat A \to \catname{Set}$. \\
...
Analogously, product sketches with larger cones correspond to infinitary varieties. We'll now look at examples of sketches with non-discrete cones:
...
Note that models of a finite limit sketches can be axiomatised in finitary first-order logic. This will contrast finite mixed sketches.

We note that limits of models of essentially algebraic theories can still be computed on the level of the underlying sets, sort-by-sort. This translates back to the following observation:
...
The category of fields can be neither complete nor cocomplete, as there are only homomorphisms between fields of the same characteristic. However, it has connected limits, so for example the intersection of two subfields is again a subfield. Note how the connectedness of the diagram precisely takes care of the condition on the characteristics. \\
...
This is not the case any more for arbitrary cocones in $\dist C$. Recall the coequaliser cocone in the sketch for connected graphs from the introduction
\[
\xymatrix{
& 1 \\
...
Worse, while models of finite limit sketches could be described in finitary first-order logic, this is no longer true for finite mixed sketches. Connected graphs are not first-order axiomatisable by the compactness theorem. 
